...
res=$(eval "datamon1 bundle get --repo ${REPO} $srcBundle" 2>&1|grep -v '{"level"'|grep -v '^Using')
if [[ $? -ne 0 || ${res} =~ "didn't find" ]] ; then
...
function setLabels {
  ...
  while read l ; do
    ...
    if [[ ! -z ${label} && ${label} != ${DESTLABEL} ]] ; then
      ...
      eval "datamon2 label set --repo ${REPO} --bundle \"${bundleID}\" --label \"${label}\" $targetContext"
      if [[ $? -ne 0 ]] ; then
  ...
  if [[ ! -z ${DESTLABEL} ]] ; then
    ...
    eval "datamon2 label set --repo ${REPO} --bundle \"${bundleID}\" --label \"${DESTLABEL}\" $targetContext"
    if [[ $? -ne 0 ]] ; then
...
if [[ -z ${done} ]] ; then
  ...
  eval "datamon2 repo get --repo ${REPO} $targetContext"
  if [[ $? -ne 0 ]] ; then
    echo "INFO: Repo ${REPO} does not exist in the V2 config yet. Creating it"
    eval "datamon2 repo create --repo ${REPO} --description \"${description}\" $targetContext"
    if [[ $? -ne 0 ]] ; then
 ...
 eval "datamon2 bundle get --repo ${REPO} --bundle \"${bundleID}\""
 if [[ $? -eq 0 ]] ; then
...
eval "datamon1 bundle download --repo ${REPO} --bundle \"${bundleID}\" srcBundle --destination ${dest}"
if [[ $? -ne 0 ]] ; then
...
eval "datamon2 bundle upload --repo ${REPO} --path ${dest} --bundle \"${bundleID}\" --message \"${message}\" $targetContext"
if [[ $? -ne 0 ]] ; then
