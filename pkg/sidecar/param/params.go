...
func fuseParamsGlobalString(fuseParams FUSEParams) (string, error) {
	...
	if fuseParams.Globals.ConfigBucketName == "" {
		return rv, errors.New("config bucket not set")
	}
	rv, err = appendToParamString(rv, "b",
		fuseParams.Globals.ConfigBucketName)
	if err != nil {
		return rv, fmt.Errorf("build parameter string: %v", err)
	}
	if fuseParams.Globals.ContextName == "" {
		return rv, errors.New("context not set")
	}
	rv, err = appendToParamString(rv, "a",
		fuseParams.Globals.ContextName)
	if err != nil {
		return rv, fmt.Errorf("build parameter string: %v", err)
	}
